...
export function ParticipantSignalBadges({ participants, myId }: { participants: ParticipantState[]; myId: string }) {
  ...
  return (
    <ul className="pointer-events-none absolute right-4 top-4 z-20 flex max-w-[60%] flex-col items-end gap-1.5" aria-label={tx("Participant signals")}>
      {active.map((p) => (
        <li
          key={p.participantId}
        ...
        >
          {p.handRaised && <Hand size={13} className="text-amber-300" aria-label={tx("Hand raised")} />}
