    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <Card className="p-6">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-foreground">{tx("Discussion questions")}</h3>
            </div>

            {full ? (
              <>
                <p className="mb-4 text-xs text-muted-foreground">
                  {tx("Your instructor will guide the conversation around these. Some were drafted with AI assistance and approved by the instructor.")}
                </p>
              ...
              <>
                <div className="mb-4 flex items-start gap-2 rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-700">
                  ...
                  {tx("Full discussion questions unlock once your booking is confirmed. Here's a preview of what you'll practise:")}
                </div>
                <ol className="space-y-3">
          ...
          {full && (
            <Card className="p-6">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-foreground">{tx("Vocabulary to use")}</h3>
              </div>
        ...
        <div className="space-y-6">
          <Card className="p-5">
            <h3 className="mb-1 font-display font-bold text-foreground">{tx("Subtopics covered")}</h3>
            <p className="mb-3 text-xs text-muted-foreground">{tx("What the instructor plans to walk through.")}</p>
          ...
          <Card className="p-5">
            <h3 className="mb-3 font-display font-bold text-foreground">{tx("Your instructor")}</h3>
          ...
          <Card className="p-5">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="font-display font-bold text-foreground">{tx("Pick a time")}</h3>
            </div>
            ...
            {slotsQuery.data && openSlots.length === 0 && (
              <p className="mb-3 text-xs text-muted-foreground">{tx("No open times right now — check back soon.")}</p>
            )}
            ...
            <Button className="w-full" disabled={!picked || createBooking.isPending} onClick={book}>
              {createBooking.isPending ? tx("Booking…") : picked ? tx("Confirm booking") : tx("Select a time")}{" "}
              <ArrowRight size={16} />
            </Button>
          </Card>
        </div>
