      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <Card className="p-6">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground">
            <GraduationCap size={16} className="text-indigo-600" /> {tx("Homework from your last session")}
          </div>
          {latestReportId && homework.length > 0 ? (
            <HomeworkChecklist items={homework} />
          ) : (
            <p className="text-sm text-muted-foreground">
              {tx("After your next session, your AI tutor's recommended drills will appear here to work through.")}
            </p>
          )}
        </Card>
    ...
    <Card className="p-6">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <BookMarked size={16} className="text-purple-600" /> {tx("Vocabulary")}
        </div>
      ...
      <div className="flex flex-col items-center gap-4 rounded-2xl bg-purple-50/50 py-10">
        ...
        <span className="text-xs text-muted-foreground">{tx("Try to use it in a sentence out loud.")}</span>
      </div>
      <div className="mt-4 flex gap-2">
        <Button variant="ghost" className="flex-1" onClick={() => next(false)}>
          <RefreshCw size={15} /> {tx("Review again")}
        </Button>
        <Button className="flex-1" onClick={() => next(true)}>
          <Check size={15} /> {tx("I know it")}
        </Button>
      </div>
    </Card>
  );
...
function PracticePhrases({ phrases }: { phrases: string[] }) {
  ...
  return (
    <Card className="p-6">
      <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground">
        <MessageSquareText size={16} className="text-sky-600" /> {tx("Speaking practice")}
      </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">{tx("Answer this out loud as if you were in a session.")}</p>
      <Button variant="soft" size="sm" className="mt-3" onClick={() => setI((n) => (n + 1) % shuffled.length)}>
        {tx("Next phrase")} <ArrowRight size={15} />
      </Button>
    </Card>
  );
}
