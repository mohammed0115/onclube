    ...
    <DashboardLayout>
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Card className="p-4"><div className="mb-1 flex items-center gap-1 text-xs text-muted-foreground"><Award size={13} /> Level</div><div className="text-lg font-bold text-foreground">{s.level ?? "—"}</div></Card>
          <Card className="p-4"><div className="mb-1 flex items-center gap-1 text-xs text-muted-foreground"><Target size={13} /> Goal</div><div className="text-sm font-semibold text-foreground">{s.goalTitle ?? "—"}</div></Card>
          <Card className="p-4"><div className="mb-1 flex items-center gap-1 text-xs text-muted-foreground"><CalendarClock size={13} /> Sessions left</div><div className="text-lg font-bold text-foreground">{s.sessionsRemaining}</div></Card>
          <Card className="p-4"><div className="mb-1 text-xs text-muted-foreground">Payment</div><Badge tone={s.paymentStatus === "approved" ? "emerald" : "amber"} className="capitalize">{s.paymentStatus}</Badge></Card>
        </div>

        <Card className="p-6">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground">
            <FileBarChart size={16} className="text-indigo-600" /> Session history
          </div>
          {s.sessions.length === 0 ? (
              ...
              {s.sessions.map((x) => (
                <div key={x.id} className="flex items-center justify-between rounded-xl border border-border px-4 py-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {x.reportId && <Link to={`/student/report/${x.reportId}`} className="text-xs font-semibold text-indigo-600 hover:underline">Report</Link>}
                  </div>
                </div>
              ))}
            </div>
